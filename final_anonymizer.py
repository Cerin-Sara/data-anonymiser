...
def pii_identify(df):
    ...
    df.rename(columns={"Anonymized Tweet": "Tweet Content"}, inplace=True)
        ...
        # anonymized_tweet = anonymized_results.text
        # anonymized_tweets.append(anonymized_tweet)
    # df_ner["Anonymized Tweet"] = anonymized_tweets
...
def epsilon_differential_privacy(df, flag):
    ...
    # data = pd.read_csv('covid-dataset.csv', nrows=10,usecols=selected_attributes)
    
    sensitivity = 1.0  # Sensitivity depends on the specific query being applied
