...
def k_anonymity_algo(df):
    ...
    sensitive_attr_list = [col for col in df.columns]
    sensitive_attr = st.selectbox("Select a sensitive attribute", sensitive_attr_list)
    if st.button("Submit"):

        df_after_kanon = kanonymity.k_anonymity(df, k, sensitive_attr, quasi_identifiers)
        ...
        df_after_kanon.to_csv("anonymized.csv", index=False)
        st.write("Anonymized Dataset")
        st.write(df_after_kanon)
        data = df_after_kanon.copy()
        ...
        def create_scatter_plot(risks):
            data = pd.DataFrame({
                'Risk': risks,
                'Records': range(len(risks)),
            })
            chart = alt.Chart(data).mark_circle().encode(
                x='Records',
                y='Risk',
                size=alt.Size('Risk', scale=alt.Scale(range=[50, 50])),
...
if uploaded_file is not None:
    ...
    ('Epsilon Differential Privacy', 'k-Anonymity', 'Hybrid'))

    if method == "Hybrid":
        pii_identify(df)
        epsilon_differential_privacy(df, True)

    if method == "Epsilon Differential Privacy":
        pii_identify(df)
